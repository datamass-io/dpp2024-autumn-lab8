...
def create_index():
    ...
    fields = [
        ...
        SearchField(name="question", type=SearchFieldDataType.String,
                    searchable=True, filterable=True, sortable=True, facetable=True),
        SearchField(name="answer", type=SearchFieldDataType.String,
                    searchable=True, filterable=True, sortable=True, facetable=True),
        SearchField(name="round", type=SearchFieldDataType.String,
                    searchable=True, filterable=True, sortable=True, facetable=True),
        SearchField(name="show_number", type=SearchFieldDataType.Int32,
                    searchable=False, filterable=True, sortable=True, facetable=True),
    ]
...
def create_datasource():
    ...
    result = indexer_client.create_data_source_connection(data_source_connection=data_source)
    print(result)
...
def create_indexer():
    ...
    print("\nCreating a datasource\n\t" + indexer.name)
    results = indexer_client.create_or_update_indexer(indexer=indexer)
    print(results)


def prepare_service():
    # fix all the 4 functions below, so the prepare service succeeds
    # start with upload_data_to_blob_storage and when it succeeds you can comment it out
